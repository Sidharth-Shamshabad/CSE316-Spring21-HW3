...
export class SortTasks_Transaction extends jsTPS_Transaction {
  constructor(
    listID,
    callback,
    sortTasksFlag,
  ...
  async doTransaction() {
    let { data } = await this.sortFunction({
      variables: {
        _id: this.listID,
        sortTasksFlag: this.sortTasksFlag,
      },
    })
...
export class SortDueDates_Transaction extends jsTPS_Transaction {
  constructor(
    listID,
    callback,
    sortDueDatesFlag,
  ...
  async doTransaction() {
    let { data } = await this.sortFunction({
      variables: {
        _id: this.listID,
        sortDueDatesFlag: this.sortDueDatesFlag,
      },
    })
    if (this.sortDueDatesFlag === 0) {
...
export class SortStatus_Transaction extends jsTPS_Transaction {
  constructor(
    listID,
    callback,
    sortStatusFlag,
  ...
  async doTransaction() {
    let { data } = await this.sortFunction({
      variables: {
        _id: this.listID,
        sortStatusFlag: this.sortStatusFlag,
      },
    })
    if (this.sortStatusFlag === 0) {
